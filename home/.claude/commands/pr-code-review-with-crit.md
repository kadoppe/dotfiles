...
まず一次レビューの指摘件数（🔴 / 🟡 の内訳）を伝え、「ブラウザで確認し、返答が要るところにコメントを付けて Finish Review を押してください」と案内する。そのうえで:
...
3. **対象は「自分以外が書いたコメント」と「自分のコメントに付いた返信」だけ。** ステップ3 で登録した指摘は自分が書いたもの（`author` が `Claude Code`）なので、それ自体に返信し返さないこと
4. 返答が必要なものに返信する。3 件以上なら `--json` で一括投入する
...
5. **`--resolve` は付けない。** 解決の判断はレビュアーのものであって、こちらが勝手に閉じない
6. 再度 `crit --pr $ARGUMENTS` を実行して次のラウンドに入る。「変更を反映しました。ブラウザで確認して Finish Review を押してください」と伝える
