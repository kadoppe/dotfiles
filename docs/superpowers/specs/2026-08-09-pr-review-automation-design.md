...
5. **レビューラウンドを回す** — `crit --pr <番号>` を実行してブラウザの「Finish Review」を待つ。
   押されたら、**自分以外が書いたコメントと、自分のコメントへの返信だけ**を対象に返信し、
   再度 `crit --pr <番号>` で次のラウンドに入る。コメント 0 件で Finish されたら抜ける。
   ここに入らないと、ユーザーがブラウザでコメントを書いても消しても検知できない。
   `crit comment` は個別コメントの編集・削除ができない（追加・返信・一括投入・全消しのみ）ため、
   指摘の取り下げを求められた場合も返信で応じる。`--resolve` は付けない（解決はレビュアーの判断）。
6. **引き継ぎ** — GitHub への反映は `crit push <番号>` で行うが、**これは人が自分のタイミングで実行する**。
   コマンドから勝手に push しない。
